...
class Gmap_proxy_lib {
	...
	public function route($method)
	{
		...
		$this->EE->load->model('gmap_proxy_model');
		$this->EE->load->driver('channel_data');
		$this->EE->channel_data->api->load('gmap');
		
		$this->model = $this->EE->gmap_proxy_model;
		
		if(!method_exists($this, $method))
		{
			return $this->response(array(
				'error' => '\''.$method.'\' is not a valid endpoint.'
			), 405);
		}
	...
	public function geocode()	
	{
		...
		{
			$limit	  = $this->EE->input->get('limit', TRUE);
			$offset	  = $this->EE->input->get('offset', TRUE);
